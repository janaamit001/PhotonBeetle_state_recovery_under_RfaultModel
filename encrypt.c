...
unsigned char ftag[ 16 ], nfstate[32], fstate[32];
	...
	uint8_t *State)
{
	...
	while (i > SQUEEZE_RATE_INBYTES)
	{
		//printf("1.Last_PHOTON_Permutation\n");
		Last_PHOTON_Permutation(State);
		memcpy(Tag_out, State, SQUEEZE_RATE_INBYTES);
		Tag_out += SQUEEZE_RATE_INBYTES;
		i -= SQUEEZE_RATE_INBYTES;
	}
	//printf("2.Last_PHOTON_Permutation\n");
	Last_PHOTON_Permutation(State);
	memcpy(Tag_out, State, i);
}
	...
	)
{
	...
	*clen = mlen + TAG_INBYTES;
	...
	//copy full state
	memcpy(nfstate, State, STATE_INBYTES);
...
static void faulty_TAG(
	uint8_t *Tag_out,
	uint8_t *State, unsigned char diff, unsigned char pos)
{
	size_t i;
	...
	while (i > SQUEEZE_RATE_INBYTES)
	{
		fault_on_PHOTON_Permutation(State, diff, pos);
		memcpy(Tag_out, State, SQUEEZE_RATE_INBYTES);
		Tag_out += SQUEEZE_RATE_INBYTES;
		i -= SQUEEZE_RATE_INBYTES;
	}
	//printf("fault_on_PHOTON_Permutation\n");
	fault_on_PHOTON_Permutation(State, diff, pos);
	memcpy(Tag_out, State, i);
	//copy full faulty state
	memcpy(fstate, State, STATE_INBYTES);
}
	...
	)
{
	...
	*/
	...
	{
		XOR_const(State, 1);
		faulty_TAG(T_tmp, State, diff, pos);
		if (memcmp(T_tmp, T, TAG_INBYTES) != 0) return TAG_UNMATCH;
		*mlen = 0;
		return TAG_MATCH;
	}
	...
	/*printf("In the decryption call:\n ");
	
	printf("queried faulty Tag::\n");
	for( short i = 0; i < 16; ++i )
		printf("%02x ", T[ i ]);
		
	printf("\n\n");
	
	printf("Tags and their differences after injecting nibble difference::\n");
	for( short i = 0; i < 16; ++i )
		printf("%02x ", T_tmp[ i ]);
